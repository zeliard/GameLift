...
/// from Seungmo Koo's Grand Object-bound Dispatcher: https://github.com/zeliard/Dispatcher
...
public:
	JobQueue() : mHead(&mStub), mTail(&mStub)
	{
		...
		CRASH_ASSERT(mHead.is_lock_free());
	}
	~JobQueue() {}
...
public:
	...
	int ReleaseRefForThis()
	{
		return mRefCount.fetch_sub(1);
	}
