...
bool IocpManager::Initialize()
{
	...
	{
		GConsoleLog->PrintOut(false, "[DEBUG] listen socket IOCP register error: %d\n", GetLastError());
		return false;
	}
...
void IocpManager::StopIoThreads()
{
	...
	for (int i = 0; i < mIoThreadCount; ++i)
	{
		...
		{
			GConsoleLog->PrintOut(false, "PostQueuedCompletionStatus Error: %d\n", GetLastError());
		}
...
void IocpManager::PostIocpTask(void* context)
{
	if (FALSE == PostQueuedCompletionStatus(mCompletionPort, 0, (ULONG_PTR)CK_IOCP_TASK, (LPOVERLAPPED)context))
	{
		GConsoleLog->PrintOut(false, "PostQueuedCompletionStatus Error: %d\n", GetLastError());
		CRASH_ASSERT(false);
	}
